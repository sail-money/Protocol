...
**Operator responsibility:** the vault/pool allowlist is the operative trust decision. Pinning binds the *calldata* recipient/owner and the kernel executes exactly the evaluated calldata, so proceeds return to the account only when the allowlisted venue honours standard ERC-4626 / Aave semantics — actually paying the named `receiver`/`to` and burning only the `owner`'s shares. The permission cannot force a byte-compliant-but-dishonest venue to honour them; allowlist only venues trusted to implement the standard faithfully. This is a trust boundary, not a template gap: the template enforces the calldata pin, the target allowlist, the per-tx cap, and fail-closed evaluation, and nothing beyond the call it authorises. Separately, the `redeem` cap counts shares (oracle-free by design), whose asset value floats with the share price and is unbounded above — an operator needing a fixed value ceiling per exit should cap on the `withdraw` path (assets), not `redeem` (shares).
