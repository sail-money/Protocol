...
\end{tikzpicture}
\caption{Manager dispatch flow. The Permission Signer authorizes the mandate by EIP-712 signature. The Manager submits a transaction. The kernel evaluates each registered permission via \code{staticcall}; if every permission returns \code{true}, the kernel forwards the call to the Safe through the Safe module interface. When the Manager calls \code{collectFees}, the kernel consults the registered Fee Policy and splits the result.}
\label{fig:flow}
\end{figure}
