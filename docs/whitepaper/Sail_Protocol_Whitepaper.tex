...
\end{equation*}
The active rate is held in SailGovernance, bounded above by the immutable cap of 0.01~ETH (\texttt{MAX\_PERMISSION\_FEE\_WEI}), and tunable through the timelock. Excess \texttt{msg.value} is refunded. The fee is denominated in native ETH with no oracle dependency; governance retunes the rate as ETH price moves.
...
\begin{itemize}
  ...
  \item The maximum permission registration fee: $\texttt{MAX\_PERMISSION\_FEE\_WEI} = 0.01$~ETH.
\end{itemize}
