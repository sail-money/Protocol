...
\addlinespace[2pt]
WithdrawPermission & ERC-20 movements pinned to one configured recipient. Token allowlist, size cap, \texttt{transferFrom} source must be the account. \\
\addlinespace[2pt]
...
\begin{itemize}
  ...
  \item The maximum permission registration fee: a constitutional ceiling of $0.01$ of the chain's native token (0.01~ETH on Ethereum and ETH-denominated chains) that the immutable \texttt{MAX\_PERMISSION\_FEE\_WEI}, set per deployment, can never exceed.
\end{itemize}
