...
Permissions are user-deployed Solidity contracts. The kernel evaluates each one via \code{staticcall} under a per-permission gas cap and dispatches the call to the Safe only if every permission returns \code{true}. Because permissions are arbitrary Solidity, any DeFi primitive can be a permission. The protocol covers, by construction, everything DeFi can do.
...
\subsection{The Mandate Object}

In Sail, the mandate is not a document. It is the union of two onchain objects: a Safe held by the Owner, which holds the SMA's assets, and a list of registered permissions --- contracts implementing \code{IPermission} --- against that Safe.

When the Manager submits a transaction, the kernel calls \code{evaluate()} on each registered permission. The transaction is dispatched to the Safe only if every permission returns \code{true}. A single rejection terminates the dispatch.
...
\code{PermissionFactory} & Untrusted UX orchestrator. Bundles permission configuration and registration into single transactions. Holds no protocol-level privileges.\\
...
Interfaces & \code{IPermission}, \code{IConfigurablePermission}, \code{IFeePolicy}, \code{IOracle}.\\
\midrule
...
\bottomrule
\end{tabularx}
\caption{Components in the v1 audit scope.}
...
% Top row
\node[authoritybox] (signer) at (0, 0) {Permission Signer};
\node[authoritybox] (manager) at (6.5, 0) {Manager\\\footnotesize(typically an agent)};

% Middle - kernel
\node[kernelbox] (kernel) at (3.25, -2.6) {\code{\large SailKernel}\\\footnotesize trusted core};

% Bottom row
\node[bottombox] (perms) at (0, -5.2) {Permissions\\\footnotesize user-deployed};
\node[bottombox] (safe) at (6.5, -5.2) {Safe\\\footnotesize custody};

% Authority arrows (into kernel from top)
\draw[flowarrow] (signer.south) -- ($(kernel.north west)+(0.5,0)$)
  node[midway, fill=white, inner sep=2pt, font=\footnotesize]{EIP-712 mandate};
\draw[flowarrow] (manager.south) -- ($(kernel.north east)+(-0.5,0)$)
  node[midway, fill=white, inner sep=2pt, font=\footnotesize]{dispatch};

% Execution arrow to permissions, return bool
\draw[flowarrow] ($(kernel.south west)+(0.4,0)$) -- (perms.north)
  node[midway, fill=white, inner sep=2pt, font=\footnotesize]{\code{staticcall}};
\draw[returnarrow] (perms.north east) to[bend right=18] node[midway, fill=white, inner sep=2pt, font=\footnotesize]{\code{bool}} ($(kernel.south)+(-0.3,0)$);

% Execution arrow to Safe
\draw[flowarrow] ($(kernel.south east)+(-0.4,0)$) -- (safe.north)
  node[midway, fill=white, inner sep=2pt, font=\footnotesize]{\code{execModule}};

\end{tikzpicture}
\caption{Manager dispatch flow. The Permission Signer authorizes the mandate by EIP-712 signature. The Manager submits a transaction. The kernel evaluates each registered permission via \code{staticcall}; if every permission returns \code{true}, the kernel forwards the call to the Safe through the Safe module interface.}
\label{fig:flow}
\end{figure}
...
\textbf{Conjunctive semantics.} The kernel iterates over every registered permission and evaluates each one. The transaction is dispatched only if every permission returns \code{true}. There is no precedence, no override, no special-case shortcut. The mandate is the conjunction of all registered permissions.
...
Sail's approach inverts the trade-off. The kernel knows nothing about DeFi venues. It calls \code{evaluate()} on a permission contract and respects the answer. The structural guarantees (\code{staticcall}, gas cap, fail-closed, conjunctive semantics) protect the kernel from the permission; everything the permission expresses inside that envelope is the author's responsibility, not the kernel's.
...
To demonstrate the permission pattern across common DeFi primitives, Sail ships a starter set of example permission templates in the v1 audit scope. Table~\ref{tab:templates} lists them.
...
\bottomrule
\end{tabularx}
\caption{Example permission templates included in the v1 audit scope.}
\label{tab:templates}
\end{table}
...
\textbf{Configuration.} For shared templates, the Permission Signer signs a configure instruction containing the template-specific params blob. Any caller may submit the signed call; the canonical orchestrator is the \code{PermissionFactory}, but it holds no special privilege.
...
The storage variable \code{permissionRegistrationFee} is held in \code{SailGovernance}, bounded above by an immutable constitutional cap, and tunable by governance through a timelock. Excess \code{msg.value} on a registration call is refunded to the caller. The fee is denominated in native ETH and has no oracle dependency; governance is expected to retune the rate periodically as ETH price moves.
...
\textbf{The protocol cut is set to zero at protocol launch.} Fee 2 is built into the kernel from v1 but disabled by default; activation will be a separate governance decision tied to the readiness of the Sail Marketplace (Section~\ref{sec:marketplace}).
...
\code{IFeePolicy} is an interface, not a fixed contract. Sail v1 ships with one reference implementation, \code{StandardFeePolicy}, which provides management fee on AUM and performance fee above a high-water mark. The reference policy uses a manager-attested NAV model: the manager reports the portfolio value at fee collection time. This is appropriate for self-managed SMAs where the manager and the LP are the same party (see Section~\ref{sec:trust}).
...
None of these are part of the v1 audit scope. They are the work that follows once the Marketplace layer is built and the v1 audit is complete.
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  \item The maximum protocol cut on manager-collected fees.
  \item The maximum permission registration fee, set at deployment.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=4pt]
  \item \textbf{Custody isolation.} The kernel cannot transfer Safe assets except through a manager dispatch that satisfies every registered permission. The kernel has no direct write access to the Safe outside the module dispatch path.
  \item \textbf{Conjunctive evaluation.} A manager dispatch succeeds only if every registered permission returns \code{true}. A single rejection terminates the dispatch.
...
\subsection{Audit Posture}

The v1 audit scope is divided into two groups: a trusted core, and a set of example permission templates plus the reference fee policy. The protocol has been engineered for external audit, with a comprehensive test suite including a dedicated red-team adversarial layer that exercises the specific attack vectors identified in pre-audit security review.
...
The path from v1 to broad accessibility has two stages.
...
Permission & A Solidity contract implementing \code{IPermission}. The kernel evaluates each registered permission on every manager dispatch.\\
Shared template & A permission contract that serves multiple accounts via per-account configuration storage; the recommended deployment pattern.\\
...
\rule{\textwidth}{0.4pt}\\
\small This document describes Sail Protocol v1, audit-prep state, May 2026. The protocol has not yet been externally audited and is not deployed on mainnet. Information in this document is provided for technical evaluation. Nothing herein constitutes investment advice, an offer to sell, or a solicitation to buy any security or asset.
