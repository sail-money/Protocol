...
**0-decimal fee assets.** For a fee asset with 0 decimals, this minimal `1` base unit is a whole token. It is a bounded, one-time, by-design tradeoff: it occurs only on the first post-reattach window when the management leg rounds to zero, and the 1 unit flows to the normal authorized recipients (the treasury / distributor / `feeManager` split), never to an unauthorized party. Removing it would require a coupled kernel change to accept a zero-fee reanchor — which the kernel deliberately does not do (`collectFees` rejects `grossFee == 0`). Operators choosing a 0-decimal fee asset should be aware of this one-time unit.
...
### Fee-Manager Operator Requirement — Never Set the Policy as a Safe's Fallback Handler

The fee-manager-only setters (`proposeFeeManager`, `setDistributor`, `setManagementFeeBps`, `setPerformanceFeeBps`, `setDistributorBps`, `seedHighWaterMark`) authorize on `msg.sender == feeManager`. If `feeManager` is a Safe and that Safe's **fallback handler is set to the policy contract** (or to any handler that forwards calls into it), an external caller can relay a call through the Safe's fallback path so it arrives with `msg.sender == feeManager` — letting them, at worst, propose a new fee manager (then accept it) and redirect the manager fee. 

**Operator requirement:** never set a `feeManager` Safe's fallback handler to the policy. With an ordinary EOA `feeManager`, or a Safe using the standard `CompatibilityFallbackHandler`, there is no exposure — the dangerous state is a `feeManager` Safe whose fallback handler points at the policy, which an attacker cannot induce. This mirrors the kernel's "never set the kernel as a Safe's fallback handler" requirement.

**Engineering note (do not "fix" this with a calldata-length guard).** An exact-calldata-length guard is **not** a valid mitigation for this class on the single-address-argument setters (e.g. `proposeFeeManager`, `setDistributor`). A Safe FallbackManager relay appends the caller's 20 bytes, and for a one-address-argument function those appended bytes land exactly in the decoded address slot — so a length-matched relayed call both passes the guard *and* decodes the attacker as the argument. The only effective in-contract defense is a fallback-handler storage check (staticcall the Safe's fallback-handler slot and reject if it equals the policy). That is deliberately **not** implemented: the precondition is an attacker-uninducible misconfiguration, and `StandardFeePolicy` is a replaceable policy (an account can switch to another instance via `setFeePolicy`). A future change must not re-introduce a length guard here under the impression that it closes this surface.
