...
contract BoundedWithdrawPermissionTest is Test {
    ...
    function _ctx(address token, bytes memory data) internal pure returns (Context memory) {
        bytes4 sel;
        if (data.length >= 4) {
    ...
    function _ctxWithValue(address token, bytes memory data, uint256 ethValue)
        internal pure returns (Context memory c)
    {
        c = _ctx(token, data);
        c.value = ethValue;
    }
    ...
    function test_TransferFrom_FromAddressIrrelevant() public view {
        // The `from` field should not affect the outcome
    ...
    function test_Transfer_UnknownToken() public view {
        ...
        bytes memory data = _transfer(SAFE, 100e18);
        // ctx.target is the bad token
        Context memory ctx = _ctx(badToken, data);
    ...
    function test_TransferFrom_UnknownToken() public view {
        ...
        assertFalse(perm.evaluate(data, _ctx(badToken, data)));
    }
    ...
    function test_UnknownSelector() public view {
        // approve(address,uint256) — valid ERC-20 call but not transfer/transferFrom
    ...
    function testFuzz_UnknownSelector(bytes4 sel) public view {
        ...
        // Build fake calldata with 100 bytes payload so length checks pass
    ...
    function test_SetMaxAmountPerTx_TakesEffectOnEvaluate() public {
        ...
        // Lower the cap below that amount
        vm.prank(SIGNER);
